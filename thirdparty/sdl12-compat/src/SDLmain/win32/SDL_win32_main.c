...
#ifdef SDL12_COMPAT_STATIC
/* The static sdl12-compat build has no DLL entry point to load SDL2. */
extern int LoadSDL20(void);

static int LoadSDL20ForMain(void)
{
	if (!LoadSDL20()) {
		MessageBoxA(NULL,
			"sdl12-compat could not initialize SDL2. Make sure SDL2.dll is next to the executable.",
